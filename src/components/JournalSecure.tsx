  ...
  if (loadingConfig) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-6">
        <p className="text-indigo-300 font-serif italic animate-pulse">Checking lock...</p>
      </div>
    );
  }
  ...
  if (!isUnlocked) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 px-6">
        ...
        >
          <div className="w-16 h-16 bg-[#3A3A5E]/50 rounded-full flex items-center justify-center mx-auto mb-6">
          ...
          <div className="flex justify-center gap-4 mb-10">
            {[0, 1, 2, 3].map((i) => (
              <div 
                key={i}
                className={`w-3 h-3 rounded-full border border-indigo-400/50 transition-colors ${pin.length > i ? 'bg-indigo-400' : ''}`}
              />
            ))}
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[1, 2, 3, 4, 5, 6, 7, 8, 9, '', 0, 'del'].map((num, i) => (
              <button
                ...
                className={`h-12 rounded-xl text-lg font-light flex items-center justify-center transition-colors ${num !== '' ? 'hover:bg-white/5 active:bg-white/10' : ''}`}
              >
    ...
    <div className="p-6 pb-32">
      <div className="flex justify-between items-center mb-8">
        ...
        <button 
          ...
          className="flex items-center gap-2 px-4 py-2 bg-indigo-500 text-white rounded-full text-sm font-medium shadow-lg hover:scale-105 active:scale-95 transition-transform"
        >
      ...
      <AnimatePresence>
        {showEditor && (
          <motion.div
            ...
            className="fixed inset-0 z-[60] bg-[#0D0D12] p-6 pt-12 flex flex-col"
          >
            <div className="flex justify-between items-center mb-8 shrink-0">
              ...
              <button 
                ...
                className="px-6 py-2 bg-indigo-500 text-white rounded-full font-medium disabled:opacity-30"
              >
                Soul Keep
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto pb-6">
              <div className="mb-6 p-4 rounded-2xl bg-indigo-500/10 border border-indigo-500/20">
                <div className="flex justify-between items-center mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] uppercase tracking-widest text-indigo-300 font-bold">Reflection Prompt</span>
                  </div>
                  <button 
                    onClick={() => setPromptIndex((prev) => (prev + 1) % CBT_PROMPTS.length)}
                    className="p-1 hover:bg-white/5 rounded-full"
                  >
                    <RefreshCcw className="w-3 h-3 text-indigo-300" />
                  </button>
                </div>
                <p className="text-sm text-indigo-100/80 italic font-serif">"{CBT_PROMPTS[promptIndex]}"</p>
              </div>

              <input
                autoFocus
                placeholder="Title your moment..."
                className="w-full text-3xl font-serif bg-transparent outline-none mb-6 placeholder:opacity-20 text-white"
              ...
              <textarea
                ...
                className="w-full min-h-[300px] text-lg font-light bg-transparent outline-none border-none resize-none placeholder:opacity-20 leading-relaxed text-white"
                value={newEntry.content}
                onChange={(e) => setNewEntry({ ...newEntry, content: e.target.value })}
              />
            </div>

            <div className="shrink-0 pt-4 border-t border-white/10 flex items-center gap-4 overflow-x-auto pb-4">
              <span className="text-xs uppercase tracking-widest opacity-40 shrink-0">Mood:</span>
              {(['calm', 'happy', 'overwhelmed', 'sad', 'peaceful'] as const).map(m => (
                <button
                  key={m}
                  onClick={() => setNewEntry({ ...newEntry, mood: m })}
                  className={`px-3 py-1.5 rounded-full text-[10px] uppercase tracking-widest border border-white/10 transition-colors shrink-0 ${newEntry.mood === m ? 'bg-indigo-500 text-white border-indigo-500' : 'opacity-40 hover:opacity-100'}`}
                >
                  {m}
                </button>
              ))}
